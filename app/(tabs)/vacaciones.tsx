...
function CreateVacationModal({ visible, contractId, employeeId, onClose }: CreateModalProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vacations', contractId] });
      handleClose();
    },
  ...
  function handleSubmit() {
    ...
    mutation.mutate({ start_date: startDate, end_date: endDate, reason: reason.trim(), contract_id: contractId });
  }
...
function EditVacationModal({ item, contractId, onClose }: EditModalProps) {
  const queryClient = useQueryClient();
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [reason, setReason] = useState('');
...
function VacacionesContent({ employeeId }: { employeeId: number }) {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['vacations', selectedContractId],
    queryFn: () => getContractVacations(selectedContractId!),
    ...
    <View style={styles.root}>
      ...
      {!loadingContracts && contracts.length === 0 && (
        <View style={styles.centered}>
          ...
          <Text style={styles.emptySubtext}>Necesitas un contrato activo para solicitar vacaciones</Text>
        </View>
      )}
